...
| **Auto-Ascend @ 1 trillion** | Grabs the *"When the cookies ascend just right"* achievement. Fires when you **cross** 1 trillion cookies baked **this ascension** — enable it on a fresh run; if you're already past 1T it waits for your next ascension rather than firing the instant you enable it. Then it sets the bank to exactly 1,000,000,000,000 the instant before committing a real ascension (you can't land there naturally — CpS jumps the bank in big steps). One-shot; greys out once the achievement is earned. *(requires the achievement still unearned)* |
...
```
javascript:(function(){Game.LoadMod('https://cdn.jsdelivr.net/gh/price-gaines/cookie-minmax@v0.11.4/src/minmax.js')})();
```

**Console** — paste into the browser devtools console (F12):

```js
Game.LoadMod('https://cdn.jsdelivr.net/gh/price-gaines/cookie-minmax@v0.11.4/src/minmax.js');
```

**Mod Manager** — add the same `@v0.11.4` URL as a custom mod.
...
> Pin a **version tag** (`@v0.11.4`), not `@main`. Tags are immutable, so jsDelivr caches them
