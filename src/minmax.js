...
(function () {
	...
	var settings = {
		...
		autobuy:  { on: false, bankProtect: true },
	...
	var modules = [
		...
		{
			id: 'autobuy', label: 'Auto-Buy (payback)', interval: 30,
			tick: function () { autoBuyTick(); },
			menu: function () {
				return row('autobuy', 'Buy lowest payback-period item' +
					check('autobuy.bankProtect', settings.autobuy.bankProtect, 'protect Lucky! bank'));
			},
		},
	];
	...
	// Reserve enough bank that a Lucky! payout isn't capped: 15% of bank == cps*900
	// -> bank == cps*6000. (Frenzy-aware *7 is the documented upgrade path.)
	function luckyReserve() {
		return settings.autobuy.bankProtect ? Game.cookiesPs * 6000 : 0;
	}
	...
	function autoBuyTick() {
		...
		var usable = Math.max(0, Game.cookies - reserve);
		var best = null;
		...
		for (i = 0; i < Game.ObjectsById.length; i++) {
			...
			p = obj.getPrice();
			d = marginalBuilding(obj, base);
			score = pp(p, d, cps, usable);
			if (score < (best ? best.score : Infinity))
				best = { score: score, price: p, buy: (function (o) { return function () { o.buy(1); }; })(obj) };
		}
		for (i = 0; i < Game.UpgradesInStore.length; i++) {
			var u = Game.UpgradesInStore[i];
			if (!u || SKIP_POOLS[u.pool]) continue;
			p = u.getPrice();
			d = marginalUpgrade(u, base);
			score = pp(p, d, cps, usable);
			if (score < (best ? best.score : Infinity))
				best = { score: score, price: p, buy: (function (x) { return function () { x.buy(1); }; })(u) };
		}

		// FC-style: buy the global best only once affordable (otherwise save up).
		if (best && best.score < Infinity && best.price <= Game.cookies - reserve) best.buy();
	}
	...
	window.MinMax = {
		version: VERSION,
		settings: settings,
		toggle: function (id) { settings[id].on = !settings[id].on; Game.UpdateMenu(); },
